...
DragResize.prototype.apply = function(node)
{
 ...
 addEvent(node, 'mouseout', function(e) { obj.mouseOut(e) } );
};
...
DragResize.prototype.mouseDown = function(e) { with (this)
{
 ...
 if (newElement && (!element || (newElement == element)))
 {
  ...
  handle = newHandle;
  if (handle && ondragstart) this.ondragstart(hRE.test(handle.className), element);
 }
}};
...
DragResize.prototype.mouseMove = function(e) { with (this)
{
 ...
 with (element.style)
 {
  ...
  if (isResize)
  {
	  if (!fixWidth)
	  {
		width =  (elmW-paddingHorizon) + 'px';
	  }
	  if (!fixHeight)
	  {
		height = (elmH-paddingVertical) + 'px';
	  }
  }
 }
 ...
 if (ondragmove) this.ondragmove(isResize, element);
...
DragResize.prototype.mouseUp = function(e) { with (this)
{
 ...
 if (handle && ondragend) this.ondragend(hRE.test(handle.className), element);
 deselect(false);
}};

DragResize.prototype.mouseOut = function(e) { with (this)
{
 // On mouseup, stop dragging, but don't reset handler visibility.
 if (!document.getElementById || !enabled) return;
...
{
 if(  this.fixWidth && this.fixHeight ) return; // by kyo, 161102
