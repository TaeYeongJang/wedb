...
function DataGrid(container, opt) {
	...
	var _config = {
		...
		onItemUpdated: function (args) {
	        ...
	        if(this.onItemUpdated_cbf)
	        	this.onItemUpdated_cbf.call(this, args);
	    },
	    onItemEditing: function (args) {
	    	//var self = this;
	        //setTimeout(function () { self._updateColPos(2); }, 1);
	    	if(this.frozen > 0)
	    		setTimeout(function (self, cols) { self._updateColPos(cols); }, 1, this);

	        if(this.onItemEditing_cbf)
	        	this.onItemEditing_cbf.call(this, args);
	    },
	    onRefreshed: function (args) {
	    	if(this.frozen > 0)
	    		this._updateColPos();

	        if(this.onRefreshed_cbf)
	        	this.onRefreshed_cbf.call(this, args);
	    },
		_updateColPos: function() {
			var cols = this.frozen;
		...
		enableFrozenColumns: function() {
			if(!(this.frozen > 0))
				return;
			$('.jsgrid-grid-body').scroll(function () {
				_grid.jsGrid("_updateColPos");
			});
		},
		//------------------------------------------------
	...
	if(opt.onItemUpdated) {
		opt.onItemUpdated_cbf = opt.onItemUpdated;
		delete opt.onItemUpdated;
	}
	if(opt.onItemEditing) {
		opt.onItemEditing_cbf = opt.onItemEditing;
		delete opt.onItemEditing;
	}
	if(opt.onRefreshed) {
		opt.onRefreshed_cbf = opt.onRefreshed;
		delete opt.onRefreshed;
	}
	...
	if(opt.frozen > 0) {
		_grid.jsGrid("enableFrozenColumns");
	}
